...
function setupSocketListeners() {
  ...
  socket.on("voice-signal", async ({ userId, signal }) => {
    try {
      ...
      displayVoiceError(
        `Connection error: ${error.message || "Failed to connect to peer"}`
      );
    }
  });
  ...
  if (!localStream) {
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        throw new Error(
          "Your browser does not support accessing media devices"
        );
      }
      ...
      displayVoiceError(
        "Microphone access error: " + (error.message || "Permission denied")
      );
...
function initiateCall(userId) {
  try {
    ...
    peer.on("error", (err) => {
      ...
      displayVoiceError(
        `Connection error with peer: ${err.message || "Unknown error"}`
      );
...
function addUserToVoiceList(user, isCurrentUser = false) {
  ...
  userElement.innerHTML = `
        ...
        <div class="voice-user-name">${user.username}${
    isCurrentUser ? " (You)" : ""
  }</div>
        <div class="voice-user-status${
          isCurrentUser && isMuted ? " muted" : ""
        }">
            <i class="fas ${
              isCurrentUser && isMuted ? "fa-microphone-slash" : "fa-microphone"
            }"></i>
        </div>
    `;
...
function toggleMute() {
  ...
  if (currentUserElement) {
    const statusElement =
      currentUserElement.querySelector(".voice-user-status");
    if (statusElement) {
      if (isMuted) {
        statusElement.innerHTML = '<i class="fas fa-microphone-slash"></i>';
        statusElement.classList.add("muted");
...
function createDummyStream() {
  ...
  try {
    ...
    displayVoiceError(
      "Running in listen-only mode. You cannot speak but can hear others."
    );
  } catch (err) {
